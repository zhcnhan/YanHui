...
U_HINT = (
    "输出 JSON：\n"
    '{{"hint_md":"…"}}\n'
    "任务：给一条**方向性提示**（hint_md），帮学生自己往前走。\n"
    "**先看清是哪种情形**：\n"
    "1. 「学生作答」写着**还没有作答** → 学生**还没做题**，**没有错误可发现**。\n"
    "   **绝对不许**写「你刚才错在…」，**不许**假设他答错了；\n"
    "   请给一条**能照着下手的方向**：该先想哪个概念、该走哪一步、该注意什么。\n"
    "2. 「学生作答」是**他真写的答案** → 他做过且没做对，\n"
    "   请针对**他这次的错误**指出方向。\n"
    "**两种情形共同铁律**：**绝对不许给完整解答或最终答案**，也不许把答案藏在提示里；\n"
    "只给方向，让学生自己想出来。\n"
    "题面：{prompt}\n模式：{mode}\n学生作答：{student_answer}\n判题细节：{judge_detail}"
)
...
U_MODE_OUTLINE = (
    ...
    '"is_front_matter":false}}],"uncertain":false,"uncertain_reason":""}}\n'
    "学科：{subject_label}\n学习目标/背景：{brief}\n想要的单元数：{want_count}\n"
    "各页读到了什么：\n{pages_digest}\n"
    "{errors_block}"
    ...
    # **R77 前置章**：书前那几页（书名页/版权页/目录/凡例/序/前言/致谢/索引）仍有阅读价值
    # （凡例讲怎么用这本书、目录给出全书结构），所以**照旧排成单元**；但要标出来：
    # 这类内容不该拿"这本书是谁辑的"去考学生，所以它**只读不练**（不出题、不进费曼）。
    "**重要：书的前置内容要单独标出来。** 凡是**书名页、版权页、目录、凡例、序、前言、"
    "致谢、索引**这类「书本身」的内容（不是这门手艺的知识），把该单元的 `is_front_matter` "
    "置为 `true`，并在**标题里如实写出它是什么**（例如「凡例与全书目录结构」）；"
    "这类单元仍要照常给出学习目标与依据页。\n"
    "**书里没有这类前置内容的，一个都不许标**（不许把讲知识的正文标成前置内容）。"
)
...
U_MODE_EXERCISE = (
    ...
    # **R77 补充**：用户被一道"目录里『艮宫属土』后标的页码是几"的题气笑了。
    # 一句话判据：换成同主题的另一本书就答不出来的题，考的是「这本书」，不是「这门手艺」。
    "**出题禁区（硬约束）**：题目只能考**这门手艺本身**——概念、规则、步骤、判法、用法，"
    "以及把规则用到新情形上。判断标准一句话：**换成同主题的另一本书就答不出来的题，"
    "一律不要出**（那考的是「这本书」，不是「这门手艺」）。绝对不许出下面这些题：\n"
    "1. 问页码：第几页 / 页码是 / 在哪一页；\n"
    "2. 问目录与篇目：篇目顺序、某卷从哪篇开始、某内容在第几章/第几卷；\n"
    "3. 问版本与出版：谁辑的、谁注的、第几版、出版社、成书年代、校勘差异；\n"
    "4. 问这本书自己怎么写：凡例第几条、体例、编例；\n"
    "5. 问版式与版面：页眉页脚写了什么、编号用的是哪种数字、某处排版长什么样、"
    "有没有插图或表格、字体字号；\n"
    "6. 问教材里的例题/习题本身：教材上那道例题的答案是什么、某题在第几页；\n"
    "7. 问读到什么而非学会什么：「这一页讲了什么」「这部分内容出现在哪一页」；\n"
    "8. 答案是页码、编号、书名、人名、卷次这类**元信息**的题。\n"
    ...
    out: list[PromptSpec] = [
        ...
        PromptSpec(
            ...
            user_required_placeholders=("unit_title", "key_points", "want_count", "exercise_kind",
                                        "asked_before", "pages_digest", "errors_block"),
            user_required_tokens=("exercises", "prompt", "answer", "basis_pages", "输出 JSON"),
            # **R77 补充**：这一条提示词必须一直保留"出题禁区"那段硬约束（删掉＝没营养的题会回来）
            notes="标准答案与解析都由这条提示词产出；`basis_pages` 是本模式唯一的依据形式。"
                  "**不许删掉「出题禁区」那段**：它拦住问页码/目录/版本/版式这类没营养的题，"
                  "`errors_block` 用来把上一轮被判没营养的具体原因回灌给它。",
        ),
        PromptSpec(
            "mode_judge", "图示教材 · 判对错",
